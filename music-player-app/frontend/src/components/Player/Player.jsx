...
const Player = () => {
    ...
    // Логика за преминаване към следваща песен в списъка
    const handleNext = () => {
    ...
    // Логика за преминаване към предишна песен в списъка
    const handlePrev = () => {
        if (!selectedPlaylist || selectedPlaylist.songs.length === 0) return
        const idx = selectedPlaylist.songs.findIndex((s) => s.id === currentSong?.id)
    ...
    return (
        <div className="flex flex-1 overflow-hidden">
            {/* Странична лента с управление на плейлистите */}
            <aside
            ...
            <div className="flex-1 flex flex-col px-4 pt-4 overflow-hidden">
                <div className="flex flex-row lg:h-[330px] items-stretch flex-shrink-0 w-full overflow-hidden">
                    {/* Визуален компонент на музикалния плеър */}
                    ...
                    {/* Детайли за изпълнителя или системно съобщение, ако няма избрана песен */}
