    ...
    return (
        <div className="flex-1 w-full flex flex-col items-center relative px-6 overflow-y-auto pt-24 pb-32
            ...
            <div className="relative z-20 text-center flex flex-col items-center max-w-5xl mb-28 lg:mb-48">
                ...
                <h1 className="text-7xl md:text-9xl lg:text-[10rem] font-black italic tracking-tighter text-white
                ...
                <button onClick={handleStartClick}
                    className="group relative px-20 py-6 bg-white text-black font-black uppercase tracking-[0.4em]
            ...
            <div className="relative z-20 w-full max-w-7xl px-4 animate-in fade-in slide-in-from-bottom-10
                duration-1000">
                <h2 className="text-xl md:text-3xl font-black italic text-white uppercase tracking-tighter mb-8
                ...
                <div className="flex overflow-x-auto gap-6 md:gap-[21px] py-6 px-2 -mx-2 custom-scrollbar-hide snap-x
                    ...
                    {staticArtists.map(artist => (
                        ...
                        >
                            <div className="relative w-28 h-28 md:w-40 md:h-40 rounded-full bg-white/[0.03] border
                                ...
                                <div className="absolute inset-0 flex items-center justify-center opacity-0
                                    ...
                                    <div className="w-12 h-12 bg-purple-600 rounded-full flex items-center
                                        justify-center shadow-xl transform translate-y-4 group-hover:translate-y-0
                                        transition-transform duration-500">
            ...
            {currentTrack && (
                <div
                    className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md animate-in slide-in-from-bottom-10 duration-500">
                    <div
                        className="p-3 rounded-[2rem] flex items-center gap-4 border border-white/10 bg-black/80 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)]">

                        <div className="relative shrink-0">
                            <div
                                className={`absolute inset-0 bg-purple-500/20 rounded-full blur-md ${isPlaying ? 'animate-pulse' : 'opacity-0'}`}></div>
                            <img src={currentTrack.cover} alt="cover"
                                 className={`relative w-12 h-12 rounded-full border border-white/10 object-cover ${isPlaying ? 'animate-spin-slow' : ''}`}/>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-purple-500/80 text-[10px] font-bold uppercase tracking-widest mt-1.5 flex items-center gap-1.5">
                                <span
                                    className={`w-1 h-1 rounded-full bg-purple-500 ${isPlaying ? 'animate-ping' : ''}`}></span>
                                {currentTrack.artist}
                            </p>
                        </div>
        ...
        <span className="text-gray-700 font-black italic text-sm w-6 group-hover:text-purple-500">
          {(index + 1).toString().padStart(2, '0')}
                            ...
                            <img src={track.album.cover_medium} alt=""
                                 className="w-14 h-14 rounded-lg object-cover shadow-lg group-hover:scale-105 transition-transform"/>
